                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="6"
                            class="block p-3.5 w-full text-sm text-[#355872] dark:text-[#DFD0B8] bg-[#F7F8F0] dark:bg-[#393E46] rounded-lg border border-[#9CD5FF] dark:border-[#948979] focus:ring-[#7AAACE] focus:border-[#7AAACE]">{{ old('description', $event->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div id="excluded_days_list" class="mt-3 flex flex-wrap gap-2">
                            @if ($event->excluded_days && count(json_decode($event->excluded_days, true)) > 0)
                                @foreach (json_decode($event->excluded_days, true) as $day)
                                    <span
                                        class="inline-flex items-center px-3 py-1 bg-[#9CD5FF] dark:bg-[#948979] text-[#355872] dark:text-[#DFD0B8] rounded-full text-sm">
                                        {{ \Carbon\Carbon::parse($day)->format('Y-m-d') }}
                                        <button type="button"
                                            class="ml-2 text-[#7AAACE] dark:text-[#7AAACE] hover:text-[#355872]"
                    ...
                    <div class="md:col-span-2">
                        <label for="image"
                            class="block mb-2 text-sm font-medium text-[#948979] dark:text-[#948979]">صورة الفعالية
                            (اختياري)</label>
                        @if ($event->image)
                            <div class="mb-4">
                                <img src="{{ asset('storage/' . $event->image) }}" alt="الصورة الحالية"
                                    class="w-48 h-48 object-cover rounded-lg border border-[#9CD5FF] dark:border-[#948979]">
                                <p class="mt-2 text-sm text-[#948979] dark:text-[#948979]">الصورة الحالية</p>
                            </div>
                        @endif
