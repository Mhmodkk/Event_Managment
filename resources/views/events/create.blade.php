        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('events.store') }}" x-data="{
                    ...
                    <div>
                        ...
                        <select id="country_id" x-model="country" x-on:change="onCountryChange" name="country_id"
                            ...
                            @foreach ($countries as $country)
                                <option class="bg-gray-800 text-gray-200" :value="{{ $country->id }}">
                                    {{ $country->name }}</option>
                            @endforeach
                        </select>
                        @error('country_id')
                ...
                <div class="flex justify-end">
                    <button type="submit"
                        class="text-white bg-gray-700 hover:bg-gray-600 focus:ring-4 focus:ring-gray-500 font-medium rounded-lg text-sm px-6 py-3 transition duration-200 ease-in-out transform hover:scale-105 border border-gray-600">Create
                        Event</button>
                </div>
            </form>
        </div>
