<x-main-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-[#355872] dark:text-[#DFD0B8]">إنشاء فعالية جديدة</h2>
    </x-slot>
            ...
            <form method="POST" action="{{ route('events.store') }}" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="md:col-span-2">
                        <label for="title" class="block mb-2 text-sm font-bold text-[#948979]">عنوان الفعالية <span
                    ...
                    <!-- نوع الفعالية -->
                    <div>
                        <label for="type" class="block mb-2 text-sm font-bold text-[#948979]">نوع الفعالية <span
                                class="text-red-500">*</span></label>
                    ...
                    <!-- المكان -->
                    <div>
                        <label for="location" class="block mb-2 text-sm font-bold text-[#948979]">المكان <span
                                class="text-red-500">*</span></label>
                    ...
                    <!-- تاريخ ووقت البداية -->
                    <div>
                        <label for="start_date" class="block mb-2 text-sm font-bold text-[#948979]">تاريخ ووقت البداية
                    ...
                    <!-- تاريخ ووقت النهاية -->
                    <div>
                        <label for="end_date" class="block mb-2 text-sm font-bold text-[#948979]">تاريخ ووقت النهاية
                            <span class="text-red-500">*</span></label>
                    ...
                    <!-- الكلية -->
                    <div>
                        <label for="faculty_id" class="block mb-2 text-sm font-bold text-[#948979]">الكلية <span
                                class="text-red-500">*</span></label>
                    ...
                    <!-- الوصف -->
                    <div class="md:col-span-2">
                        <label for="description" class="block mb-2 text-sm font-bold text-[#948979]">الوصف <span
                                class="text-red-500">*</span></label>
                        <textarea name="description" id="description" rows="5"
                            class="block p-4 w-full text-sm text-[#355872] dark:text-[#DFD0B8] bg-[#F7F8F0] dark:bg-[#393E46] rounded-xl border border-[#9CD5FF] dark:border-[#948979] focus:ring-2 focus:ring-[#7AAACE] focus:border-[#7AAACE] transition">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex gap-3">
                            ...
                            <button type="button" id="add_excluded_day"
                                class="px-6 py-4 bg-[#9CD5FF] dark:bg-[#948979] text-[#355872] dark:text-[#DFD0B8] rounded-xl font-bold hover:bg-[#7AAACE] dark:hover:bg-[#DFD0B8] transition shadow-md">إضافة</button>
                        </div>
                    ...
                    <div
                        ...
                        <label for="is_public" class="text-sm font-medium text-[#948979]">الفعالية مفتوحة للجمهور
                            والضيوف الخارجيين</label>
                    </div>

                    <!-- صورة الفعالية -->
                    <div class="md:col-span-2">
                        <label for="image" class="block mb-2 text-sm font-bold text-[#948979]">صورة
                            الفعالية</label>
                        <input type="file" name="image" id="image" accept="image/*"
                            class="block w-full text-sm text-[#355872] border border-[#9CD5FF] dark:border-[#948979] rounded-xl cursor-pointer bg-[#F7F8F0] dark:bg-[#393E46] file:mr-4 file:py-3 file:px-6 file:rounded-lg file:border-0 file:bg-[#7AAACE] file:text-white hover:file:bg-[#9CD5FF] transition">
                        @error('image')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex gap-3 mb-4">
                            <input type="text" id="new_tag_input"
                                class="flex-1 bg-[#F7F8F0] dark:bg-[#393E46] border border-[#9CD5FF] dark:border-[#948979] text-[#355872] dark:text-[#DFD0B8] text-sm rounded-xl focus:ring-2 focus:ring-[#7AAACE] focus:border-[#7AAACE] p-4 transition"
                                placeholder="أدخل تصنيف جديد (مثال: AI, Robotics...)">
                            <button type="button" id="add_tag_btn"
                                class="px-6 py-4 bg-[#7AAACE] hover:bg-[#9CD5FF] text-white rounded-xl font-bold transition shadow-md">إضافة</button>
                        </div>
                        <div id="tags_list" class="flex flex-wrap gap-2 mb-6"></div>
                        <input type="hidden" name="tags" id="tags_hidden">

                        <div class="text-xs text-[#948979] mb-3 font-medium">أو اختر من التصنيفات الموجودة:</div>
                            ...
                            @foreach ($tags as $tag)
                                <div
                                    ...
                                    <label for="tag-{{ $tag->id }}"
                                        class="ms-2 text-sm text-[#948979]">{{ $tag->name }}</label>
                                </div>
                            @endforeach
                        </div>
                        @error('tags')
                ...
                <div class="flex justify-end gap-4 pt-6 border-t border-[#9CD5FF] dark:border-[#948979]">
                    <a href="{{ route('events.index') }}"
                        class="px-8 py-4 text-[#355872] dark:text-[#DFD0B8] hover:bg-[#F7F8F0] dark:hover:bg-[#393E46] rounded-xl font-bold transition">إلغاء</a>
                    <button type="submit"
                        class="px-10 py-4 bg-[#7AAACE] hover:bg-[#9CD5FF] text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition transform hover:scale-[1.02]">إنشاء
                        الفعالية</button>
                </div>
            </form>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            addBtn.addEventListener('click', () => {
                ...
                if (value && !dates.includes(value)) {
                    ...
                    tag.className =
                        'inline-flex items-center px-4 py-2 bg-[#9CD5FF] dark:bg-[#948979] text-[#355872] dark:text-[#DFD0B8] rounded-xl text-sm font-medium shadow-sm';
                    tag.innerHTML =
                        `${value} <button type="button" class="ml-3 text-red-500 hover:text-red-700 font-bold" onclick="removeDate('${value}')">×</button>`;
            ...
            function renderTags() {
                ...
                tagsArray.forEach((tag, index) => {
                    ...
                    pill.className =
                        'inline-flex items-center px-4 py-2 bg-[#9CD5FF] dark:bg-[#948979] text-[#355872] dark:text-[#DFD0B8] rounded-xl text-sm font-medium shadow-sm';
                    pill.innerHTML =
                        `${tag} <button type="button" class="ml-3 text-red-500 hover:text-red-700 font-bold" onclick="removeTag(${index})">×</button>`;
