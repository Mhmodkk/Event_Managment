        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('events.store') }}" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- نوع الفعالية -->
                    <div>
                        <label for="type" class="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">نوع الفعالية</label>
                        <select name="type" id="type"
                            ...
                            <option value="lecture" {{ old('type') == 'lecture' ? 'selected' : '' }}>محاضرة علمية</option>
                            <option value="orientation" {{ old('type') == 'orientation' ? 'selected' : '' }}>محاضرة تعريفية</option>
                            ...
                            <option value="sports" {{ old('type') == 'sports' ? 'selected' : '' }}>فعالية رياضية</option>
                            <option value="hackathon" {{ old('type') == 'hackathon' ? 'selected' : '' }}>مسابقة علمية/برمجية</option>
                            <option value="job_fair" {{ old('type') == 'job_fair' ? 'selected' : '' }}>ندوة توظيف</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <!-- المكان -->
                    <div>
                        <label for="location" class="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">المكان</label>
                    ...
                    <!-- تاريخ البداية -->
                    <div>
                        <label for="start_date" class="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">تاريخ البداية</label>
                    ...
                    <!-- تاريخ النهاية -->
                    <div>
                        <label for="end_date" class="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">تاريخ النهاية</label>
                    ...
                    <!-- وقت البداية -->
                    <div>
                        <label for="start_time" class="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">وقت البداية</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">أيام العطل المستثناة (اختياري)</label>
                        <div class="flex gap-3">
                            <input type="date" id="excluded_days_input" class="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                            <button type="button" id="add_excluded_day" class="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition">
                                إضافة
                            </button>
                        </div>
                    ...
                    <!-- صورة الفعالية -->
                    <div class="md:col-span-2">
                        <label for="image" class="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">صورة الفعالية</label>
                    ...
                    <!-- التصنيفات (Tags) -->
                    <div class="md:col-span-2">
                        <label class="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">التصنيفات (Tags)</label>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    addBtn.addEventListener('click', () => {
        ...
        if (value && !dates.includes(value)) {
            ...
            tag.className = 'inline-flex items-center px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full text-sm';
            tag.innerHTML = `${value} <button type="button" class="ml-2 text-blue-600 dark:text-blue-300 hover:text-blue-800" onclick="removeDate('${value}')">×</button>`;
