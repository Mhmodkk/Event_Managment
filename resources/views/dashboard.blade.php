            ...
            <div class="bg-white dark:bg-slate-900 overflow-hidden shadow-lg sm:rounded-2xl border border-slate-200 dark:border-slate-700">
                <div class="p-8 md:p-10">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                        ...
                        <div class="px-5 py-3 rounded-full bg-blue-100 dark:bg-blue-950 text-blue-800 dark:text-blue-300 font-medium text-sm">
                            الحالة:
                            <span class="font-bold">
                                {{ auth()->user()->isAdmin() || auth()->user()->isSuperAdmin() ? 'مشرف / إداري' : 'طالب' }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                        <div class="p-6 bg-blue-50 dark:bg-blue-950 rounded-xl border border-blue-100 dark:border-blue-900 shadow-sm">
                            <p class="text-sm text-blue-700 dark:text-blue-400 uppercase font-semibold tracking-wide">حجوزاتي</p>
                        ...
                        @if (auth()->user()->isAdmin() || auth()->user()->isSuperAdmin())
                            <div class="p-6 bg-purple-50 dark:bg-purple-950 rounded-xl border border-purple-100 dark:border-purple-900 shadow-sm">
                                <p class="text-sm text-purple-700 dark:text-purple-400 uppercase font-semibold tracking-wide">الفعاليات التي أنشأتها</p>
                    ...
                    @if ((auth()->user()->isAdmin() || auth()->user()->isSuperAdmin()) && $myEvents->count() > 0)
                        <div class="mb-12">
                            <h4 class="text-xl font-bold text-slate-800 dark:text-slate-200 mb-6 tracking-tight">تتبع المشاركة في الفعاليات</h4>
                                    ...
                                    <tbody class="divide-y dark:divide-slate-700">
                                        @foreach ($myEvents as $event)
                                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                                                <td class="px-6 py-4 font-semibold text-blue-700 dark:text-blue-400">{{ $event->title }}</td>
                                                <td class="px-6 py-4">
                                                    <div class="flex justify-between text-sm mb-2 font-medium">
                                                        <span class="text-slate-700 dark:text-slate-300">{{ $event->attendings_count }} / {{ $event->num_tickets }}</span>
                                                        <span class="text-blue-700 dark:text-blue-400">{{ round(($event->attendings_count / max($event->num_tickets, 1)) * 100) }}%</span>
                                                    </div>
