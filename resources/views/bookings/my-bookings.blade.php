<x-app-layout>
    ...
    <div class="py-12 bg-[#F7F8F0] dark:bg-[#222831] min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($bookings->isEmpty())
                <div class="bg-[#F7F8F0] dark:bg-[#393E46] overflow-hidden shadow-sm sm:rounded-2xl p-12 text-center">
                    <div
                    ...
                    <h3 class="text-2xl font-bold text-[#355872] dark:text-[#DFD0B8] mb-2">لا توجد حجوزات نشطة</h3>
                    <p class="text-[#948979] dark:text-[#948979] mb-8">يبدو أنك لم تقم بحجز مقعد في أي فعالية بعد.
                        استكشف
                        الفعاليات القادمة الآن!</p>
                    <a href="{{ route('eventIndex') }}"
                        class="inline-flex items-center px-6 py-3 bg-[#7AAACE] border border-transparent rounded-xl font-semibold text-white hover:bg-[#9CD5FF] transition ease-in-out duration-150 shadow-lg shadow-[#7AAACE]/30">
                        استكشف الفعاليات
                    </a>
                </div>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($bookings as $booking)
                        <div
                            class="group bg-[#F7F8F0] dark:bg-[#393E46] rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-[#9CD5FF] dark:border-[#948979]">
                            <div
                                class="relative h-40 bg-gradient-to-br from-[#7AAACE] to-[#355872] p-6 flex flex-col justify-end">
                                <div class="absolute top-4 right-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider {{ $booking->attended_at ? 'bg-[#7AAACE]/20 text-[#355872] border border-[#7AAACE]/30' : 'bg-[#9CD5FF]/20 text-[#355872] border border-[#9CD5FF]/30' }}">
                            ...
                            <div class="p-6">
                                <div class="space-y-3 mb-6">
                                    ...
                                    <div class="flex items-center text-[#948979] dark:text-[#948979] text-sm">
                                        ...
                                        <span class="ml-2 font-bold text-[#7AAACE] dark:text-[#7AAACE]">
                                            {{ $booking->num_tickets }}
                                        </span>
                                    </div>
                                </div>
                                ...
                                @if ($booking->qr_path)
                                    <button
                                        ...
                                        class="w-full py-3 bg-[#222831] dark:bg-[#7AAACE] text-white rounded-xl font-bold hover:bg-[#393E46] dark:hover:bg-[#DFD0B8] transition-colors flex items-center justify-center gap-2">
                                ...
                                @if (
                                    ...
                                    <button
                                        ...
                                        class="mt-3 w-full py-2 bg-[#7AAACE] text-white rounded-xl hover:bg-[#9CD5FF]">
                                        قيّم الفعالية
                                    </button>
                                @endif
                            </div>
    ...
    <div id="qr-modal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 hidden">
        <div class="bg-[#F7F8F0] dark:bg-[#393E46] rounded-3xl max-w-sm w-full mx-4 overflow-hidden shadow-2xl">
            <div class="p-8 text-center">
                <h3 id="modal-title" class="text-xl font-bold text-[#355872] dark:text-[#DFD0B8] mb-2">
                    تذكرة الدخول
                </h3>
                <p class="text-[#948979] dark:text-[#948979] mb-6">
                    امسح الرمز عند بوابة الفعالية
                </p>

                <div class="bg-white p-6 rounded-2xl mb-6 border-4 border-[#7AAACE] shadow-inner">
                    <img id="qr-image" src="" alt="QR Code" class="w-56 h-56 mx-auto object-contain">
                </div>

                <button onclick="closeQrModal()"
                    class="w-full py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold transition">
                    إغلاق
                </button>
            </div>
        </div>
    ...
    <div id="rating-modal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 hidden">
        <div class="bg-[#F7F8F0] dark:bg-[#393E46] p-8 rounded-xl max-w-md w-full mx-4">
            <h3 id="rating-title" class="text-xl font-bold mb-4 text-center text-[#355872] dark:text-[#DFD0B8]"></h3>
            ...
            <form id="rating-form" method="POST" action="">
                ...
                <div class="flex justify-center gap-2 mb-6">
                    @for ($i = 1; $i <= 5; $i++)
                        <button type="button" class="star-btn text-4xl text-[#948979] hover:text-amber-400"
                            data-value="{{ $i }}">★</button>
                    @endfor
                </div>
                ...
                <textarea name="comment" rows="4"
                    class="w-full p-3 border border-[#9CD5FF] dark:border-[#948979] rounded-xl bg-[#F7F8F0] dark:bg-[#393E46] text-[#355872] dark:text-[#DFD0B8]"
                    placeholder="اكتب تعليقك (اختياري)"></textarea>

                <div class="mt-6 flex justify-center gap-4">
                    <button type="button" onclick="closeRatingModal()"
                        class="px-6 py-3 bg-[#948979] text-white rounded-xl">إلغاء</button>
                    <button type="submit" class="px-6 py-3 bg-[#7AAACE] text-white rounded-xl hover:bg-[#9CD5FF]">
                        إرسال التقييم
                    </button>
                </div>
            </form>
...
<script>
    ...
    // إغلاق المودالين بالضغط على ESC
